...
const DIFFICULTIES: Record<string, { label: string; bg: keyof typeof BG_IMAGES; color: string }> = {
  ...
  adv: { label: 'ADVANCED', bg: 'simplebg', color: '#f97316' },
  exp: { label: 'EXPERT', bg: 'simplebg', color: '#ef4444' },
  mas: { label: 'MASTER', bg: 'simplebg', color: '#a855f7' },
  ult: { label: 'ULTIMA', bg: 'ultima', color: '#000000' },
  we: { label: "WORLD'S END", bg: 'worldsend', color: '#ffffff' },
  lun: { label: 'LUNATIC', bg: 'lunatic', color: '#eab308' },
  rem: { label: 'Re:MASTER', bg: 'remaster', color: '#ec4899' },
};
              ...
              <div className="bg-white/60 rounded-xl p-4 space-y-4 shadow-sm border border-white/50">
                ...
                <div className="flex flex-wrap gap-1 max-h-32 overflow-y-auto p-2 border rounded bg-white/50 custom-scrollbar">
                   <button
                      ...
                      className={`text-xs px-2 py-1 rounded font-bold ${
                        selectedLevels.size === LEVEL_LABELS.length
                ...
                <div className="space-y-1 max-h-[500px] overflow-y-auto pr-2 custom-scrollbar">
                  <div className="grid grid-cols-3 gap-2 mb-2">
                    ...
                    <button
                      onClick={() => { setSelectedSong(null); setSpecialType('blank'); }}
                    ...
                    <button
                      onClick={() => { setSelectedSong(null); setSpecialType('text'); }}
                  ...
                  {loading ? (
                    ...
                    filteredSongs.map(song => (
                      <div 
                        key={song.id}
                        className={`p-2 rounded cursor-pointer border flex items-center gap-3 ${
                            ...
                            <div className="relative group/item w-24">
                              ...
                              <div className="absolute -top-8 -right-8 z-20 opacity-0 group-hover/item:opacity-100 transition-opacity bg-white shadow-lg rounded-lg border p-1 flex flex-col gap-1 w-24" data-html2canvas-ignore="true">
                                ...
                                {item.type === 'text' ? (
                                  ...
                                  /* Difficulty Selection for Songs */
                                ...
                                <div className="flex justify-between gap-1">
                                    <button onClick={() => handleUpdateItem(block.id, item.id, { rotation: (item.rotation || 0) + 1 })} className="p-1 hover:bg-gray-100 rounded" title="旋转">
                                        <ArrowUpDown size={12} className="rotate-90" />
                                    </button>
                                    <button onClick={() => handleUpdateItem(block.id, item.id, { isMirrorX: !item.isMirrorX })} className="p-1 hover:bg-gray-100 rounded" title="水平翻转">
                                        <ArrowLeftRight size={12} />
                                    </button>
                                    <button onClick={() => handleUpdateItem(block.id, item.id, { isMirrorY: !item.isMirrorY })} className="p-1 hover:bg-gray-100 rounded" title="垂直翻转">
                                        <ArrowUpDown size={12} />
                                    </button>
                                    {item.type !== 'text' && (
                                      <button onClick={() => handleUpdateItem(block.id, item.id, { isGreyscale: !item.isGreyscale })} className="p-1 hover:bg-gray-100 rounded" title="黑白">
                                          <ImageIcon size={12} />
                                      </button>
                                    )}
                                </div>
                              ...
                              {item.type === 'song' && item.song ? (
                                ...
                                <div 
                                  className={`w-full aspect-square flex items-center justify-center bg-white border border-gray-200 rounded-md overflow-hidden ${item.isRainbow ? 'bg-gradient-to-br from-red-500 via-yellow-500 to-blue-500' : ''}`}
                                  style={{
                                    transform: `
                                      rotate(${(item.rotation || 0) * 90}deg) 
                                      scaleX(${item.isMirrorX ? -1 : 1}) 
                                ...
                                >
                                   <textarea 
                                     className={`w-full h-full p-1 text-center resize-none border-none focus:ring-0 bg-transparent ${item.isRainbow ? 'text-white font-bold placeholder-white/70' : 'text-gray-800'}`}
                                     style={{ 
                                       fontSize: `${item.fontSize || 14}px`,
                                       paddingTop: `${(item.textOffsetY || 0) + 4}px` // Base padding + offset
                                     }}
                                     value={item.text || ''}
                                     onChange={(e) => handleUpdateItem(block.id, item.id, { text: e.target.value })}
                                     placeholder="输入文本"
                                   />
                                </div>
                              ) : (
